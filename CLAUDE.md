...
> routes, transports, and a `cdx-codex` launcher that writes
> `~/.codex/config.toml` with `wire_api = "responses"`. The CLI scripts are
> `cdx-server`, `cdx-codex`, `cdx-init`; the package is `codexproxy`; the
> user config dir is `~/.codexproxy/`; the auth-token env var is
> `CODEX_PROXY_AUTH_TOKEN` (with `ANTHROPIC_AUTH_TOKEN` kept as a deprecated
