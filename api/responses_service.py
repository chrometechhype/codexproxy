...
CODEX_SYSTEM_PROMPT = (
    ...
    "## Available tools\n"
    "You have these tools available — USE THEM instead of shell commands for file operations:\n"
    "- `read_file` / `read` / `view` — read file contents\n"
    "- `write_file` / `write` / `create_file` — create or overwrite a file\n"
    "- `edit_file` / `apply_patch` — edit existing files using structured diffs\n"
    "- `shell_command` / `shell` / `exec_command` / `bash` — run shell commands\n\n"
    "## Core rules\n"
