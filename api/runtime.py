...
def _workspace_path(settings: Settings) -> str:
    """Resolve the managed agent workspace path across the legacy/new names."""
    return getattr(settings, "codex_workspace", None) or getattr(
        settings, "claude_workspace", ""
    )


def _cli_bin(settings: Settings) -> str:
    """Resolve the CLI binary name across the legacy/new field names."""
    return getattr(settings, "codex_cli_bin", None) or getattr(
        settings, "claude_cli_bin", ""
    )


def _auth_token(settings: Settings) -> str:
    """Resolve the effective auth token, falling back to legacy env names."""
    effective = getattr(settings, "effective_auth_token", None)
    if effective:
        return effective
    legacy = getattr(settings, "anthropic_auth_token", "")
    return legacy or ""
...
class AppRuntime:
    ...
    async def startup(self) -> None:
        logger.info("Starting CodexProxy...")
    ...
    async def _start_message_handler(self) -> None:
        ...
        data_path = os.path.abspath(_workspace_path(self.settings))
        ...
        plans_dir_abs = os.path.abspath(
            os.path.join(_workspace_path(self.settings), "plans")
        )
        ...
        self.cli_manager = CLISessionManager(
            ...
            claude_bin=_cli_bin(self.settings),
            auth_token=_auth_token(self.settings),
