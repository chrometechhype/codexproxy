...
from config.server_urls import local_admin_url, local_proxy_root_url
from config.settings import Settings, get_settings
from runtime.bootstrap import build_asgi_app
...
class ServerSupervisor:
    """Own one CodexProxy server lifecycle, including config-driven restarts."""
    ...
    def run(self, *, open_admin_browser: bool | None = None) -> None:
        ...
        with self._lock:
            ...
            if self._running:
                raise RuntimeError(
                    "The CodexProxy server supervisor is already running."
                )
