...
from cli.local_http import (
    open_local_request,
    with_local_proxy_bypass,
)
from cli.proxy_auth import proxy_auth_token
from config.paths import codex_model_catalog_path
from config.server_urls import local_proxy_root_url
...
_CODEX_AUTH_ENV_KEY = "CODEX_PROXY_CODEX_API_KEY"
...
def launch(argv: Sequence[str] | None = None) -> None:
    """Launch Codex CLI with CodexProxy proxy configuration."""
...
def build_codex_launcher_command(
    *,
    ...
    """Return a Codex command with ephemeral CodexProxy provider config."""
...
def codex_config_args(*, api_url: str, model: str | None = None) -> list[str]:
    """Return Codex `-c` assignments for the ephemeral CodexProxy provider."""

    args = [
        "-c",
        _toml_assignment("model_provider", "codexproxy"),
        "-c",
        _toml_assignment("model_providers.codexproxy.name", "CodexProxy"),
        "-c",
        _toml_assignment(
            "model_providers.codexproxy.base_url", _ensure_v1_url(api_url)
        ),
        "-c",
        _toml_assignment("model_providers.codexproxy.env_key", _CODEX_AUTH_ENV_KEY),
        "-c",
        _toml_assignment("model_providers.codexproxy.wire_api", "responses"),
    ]
    if model:
