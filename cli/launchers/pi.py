...
_API_KEY_ENV = "CODEX_PROXY_PI_API_KEY"
_BASE_URL_ENV = "CODEX_PROXY_PI_BASE_URL"
_BINARY_NAME = "pi"
_DISPLAY_NAME = "Pi"
_HELP_TIMEOUT_SECONDS = 5.0
_MODEL_SCOPE = "codexproxy/**"
...
def launch(argv: Sequence[str] | None = None) -> None:
    """Launch Pi with a process-local CodexProxy provider."""
    ...
    if not extension_path.is_file():
        print(
            "CodexProxy's bundled Pi extension is missing. Reinstall CodexProxy.",
            file=sys.stderr,
        )
        raise SystemExit(1)
...
def build_pi_launcher_command(
    *,
    ...
    """Return a Pi session command with ephemeral CodexProxy provider registration."""
...
def build_pi_launcher_env(
    *,
    ...
    """Return a Pi environment containing only cdx-owned proxy variables."""

    env = with_local_proxy_bypass(
        {
            key: value
            for key, value in base_env.items()
            if not key.startswith("CODEX_PROXY_PI_")
        },
        proxy_root_url=proxy_root_url,
    )
...
def pi_binary_is_compatible(binary_path: str) -> bool:
    """Return whether an executable exposes the Pi features CodexProxy requires."""
