...
const API_KEY_ENV = "CODEX_PROXY_PI_API_KEY";
const BASE_URL_ENV = "CODEX_PROXY_PI_BASE_URL";
...
function catalogModelIds(payload: unknown): string[] {
	if (!isRecord(payload) || payload.object !== "list" || !Array.isArray(payload.data)) {
		throw new Error("CodexProxy model catalog returned an invalid response shape.");
	}
...
export function projectCodexProxyModels(payload: unknown): ProviderModelConfig[] {
	...
	if (models.length === 0) {
		throw new Error("CodexProxy model catalog contains no routable provider models.");
	}
	return models;
}
...
async function fetchCodexProxyModels(baseUrl: string, apiKey: string): Promise<ProviderModelConfig[]> {
	...
	try {
		...
		try {
			...
			if (error instanceof Error && error.name === "AbortError") {
				throw new Error(`CodexProxy model catalog timed out after ${CATALOG_TIMEOUT_MS}ms.`);
			}
			const message = error instanceof Error ? error.message : String(error);
			throw new Error(`Could not reach the CodexProxy model catalog: ${message}`);
		}

		if (!response.ok) {
			throw new Error(`CodexProxy model catalog returned HTTP ${response.status}${requestIdSuffix(response)}.`);
		}
		...
		try {
			...
			throw new Error(`CodexProxy model catalog returned invalid JSON${requestIdSuffix(response)}.`);
		}
		return projectCodexProxyModels(payload);
	} finally {
...
export default async function freeClaudeCode(pi: ExtensionAPI): Promise<void> {
	...
	const models = await fetchCodexProxyModels(baseUrl, apiKey);

	pi.registerProvider("codexproxy", {
		name: "CodexProxy",
		baseUrl,
		apiKey: `$${API_KEY_ENV}`,
		authHeader: true,
