...
`codexproxy` originally forked from a project that proxied agent CLI traffic through the Anthropic Messages API. The rebrand to `codexproxy` targets the OpenAI Responses API consumed by the Codex CLI (`codex exec`).
