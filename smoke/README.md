...
The second command skips everything unless `CODEX_PROXY_LIVE_SMOKE=1` is set, but still
...
```powershell
$env:CODEX_PROXY_LIVE_SMOKE = "1"
$env:CODEX_PROXY_SMOKE_TARGETS = "providers"
...
`CODEX_PROXY_ALLOW_NO_PROVIDER_SMOKE=1`.
...
| `voice` | generated WAV through local Whisper or NVIDIA NIM transcription | `VOICE_NOTE_ENABLED=true`, `CODEX_PROXY_SMOKE_RUN_VOICE=1` |

## Examples

```powershell
$env:CODEX_PROXY_LIVE_SMOKE = "1"
$env:CODEX_PROXY_SMOKE_PROVIDER_MATRIX = "open_router,nvidia_nim,deepseek,lmstudio,llamacpp,ollama"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:CODEX_PROXY_LIVE_SMOKE = "1"
$env:CODEX_PROXY_SMOKE_TARGETS = "ollama"
...
```powershell
$env:CODEX_PROXY_LIVE_SMOKE = "1"
$env:CODEX_PROXY_SMOKE_TARGETS = "telegram,discord,voice"
$env:CODEX_PROXY_SMOKE_RUN_VOICE = "1"
uv run pytest smoke/product -n 0 -s --tb=short
```
...
```powershell
$env:CODEX_PROXY_LIVE_SMOKE = "1"
$env:CODEX_PROXY_SMOKE_TARGETS = "openrouter_free_cli"
$env:CODEX_PROXY_SMOKE_OPENROUTER_FREE_MODELS = "nvidia/nemotron-3-super-120b-a12b:free,openai/gpt-oss-120b:free,poolside/laguna-m.1:free"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:CODEX_PROXY_LIVE_SMOKE = "1"
$env:CODEX_PROXY_SMOKE_TARGETS = "messaging,config,extensibility"
uv run pytest smoke/product -n 0 -s --tb=short
```
...
- `CODEX_PROXY_SMOKE_OPENROUTER_FREE_MODELS`: optional comma-separated OpenRouter free
  CLI matrix models that replace the default characterization set.
- `CODEX_PROXY_SMOKE_OPENROUTER_FREE_EXTRA_MODELS`: optional comma-separated OpenRouter
  free CLI matrix models appended to the default or replacement set.
- `CODEX_PROXY_SMOKE_TIMEOUT_S`: per-request/subprocess timeout, default `45`.
- `CODEX_PROXY_SMOKE_CLAUDE_BIN`: Claude CLI executable name, default `claude`.
- `CODEX_PROXY_SMOKE_TELEGRAM_CHAT_ID`: Telegram chat/user ID for send/edit/delete.
- `CODEX_PROXY_SMOKE_DISCORD_CHANNEL_ID`: Discord channel ID for send/edit/delete.
- `CODEX_PROXY_SMOKE_INTERACTIVE=1`: enables manual inbound Telegram/Discord checks.
- `CODEX_PROXY_SMOKE_RUN_VOICE=1`: allows voice transcription backends to load/run.
...
- `target_disabled`: skipped because `CODEX_PROXY_SMOKE_TARGETS` intentionally selected
  a different target.
