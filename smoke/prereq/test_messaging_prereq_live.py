...
def test_telegram_bot_api_permissions(smoke_config: SmokeConfig) -> None:
    ...
    chat_id = os.getenv("CODEX_PROXY_SMOKE_TELEGRAM_CHAT_ID") or (
...
def test_discord_bot_api_permissions(smoke_config: SmokeConfig) -> None:
    ...
    channel_id = os.getenv("CODEX_PROXY_SMOKE_DISCORD_CHANNEL_ID")
    ...
    if not channel_id:
        pytest.skip(
            "CODEX_PROXY_SMOKE_DISCORD_CHANNEL_ID or ALLOWED_DISCORD_CHANNELS required"
        )
...
def test_interactive_inbound_messaging_requires_explicit_mode(
    ...
    if not smoke_config.interactive:
        pytest.skip(
            "set CODEX_PROXY_SMOKE_INTERACTIVE=1 for manual inbound messaging checks"
        )
    pytest.skip(
