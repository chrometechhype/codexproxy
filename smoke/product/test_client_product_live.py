    ...
    with SmokeServerDriver(
        ...
        result = subprocess.run(
            [
                ...
                "--no-approve",
                "--print",
                "Reply with exactly CODEX_PROXY_SMOKE_PI",
            ],
            cwd=tmp_path,
            env=env,
            check=False,
...
def test_claude_cli_adaptive_thinking_e2e(
    ...
    with SmokeServerDriver(
        ...
        result = ClientProtocolDriver.run_claude_prompt(
            ...
            config=smoke_config,
            cwd=tmp_path,
            prompt="think hard, then reply with exactly CODEX_PROXY_SMOKE_CLI",
        )
        server_log = server.log_path.read_text(encoding="utf-8", errors="replace")
    ...
    assert "CODEX_PROXY_SMOKE_CLI" in result.stdout
...
def test_claude_cli_provider_error_e2e(
    ...
    if not claude_bin:
        pytest.skip(f"missing_env: Claude CLI not found: {smoke_config.claude_bin}")
    broken_model = "lmstudio/cdx-smoke-failing-model"
    ...
    with (
        ...
        result = ClientProtocolDriver.run_claude_prompt(
            ...
            config=smoke_config,
            cwd=tmp_path,
            prompt="Reply with exactly CODEX_PROXY_SMOKE_UNREACHABLE.",
