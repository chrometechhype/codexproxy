...
from codexproxy.messaging.platforms.factory import create_messaging_components
from codexproxy.providers.runtime import build_provider_config
...
def _isolated_config_env(home: Path) -> dict[str, str]:
    ...
    for field in Settings.model_fields.values():
        ...
        if isinstance(alias, str):
            env.pop(alias, None)
    env.pop("CODEX_PROXY_ENV_FILE", None)
    env["HOME"] = str(home)
...
def _write_managed_config(home: Path, text: str) -> None:
    ...
    path.write_text(f"CODEX_PROXY_CONFIG_SCHEMA=1\n{text}", encoding="utf-8")
...
def test_removed_env_migration_e2e(smoke_config: SmokeConfig, tmp_path) -> None:
    ...
    env = _isolated_config_env(home)
    env["CODEX_PROXY_ENV_FILE"] = str(env_file)
    script = (
        "from codexproxy.config.loader import get_settings; print(get_settings().model)"
    )
    result = run_captured_text(
        cmd_python_c(script),
        cwd=smoke_config.root,
