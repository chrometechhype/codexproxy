...
from codexproxy.core.anthropic import anthropic_error_type_for_failure
...
from codexproxy.core.async_iterators import try_close_async_iterator
from codexproxy.core.diagnostics import safe_exception_message
from codexproxy.core.failures import find_execution_failure
from codexproxy.core.json_types import JsonObject
from codexproxy.core.trace import close_stream_input, trace_event
...
def _trace_response_cleanup_failure(operation: str, exc: BaseException) -> None:
    trace_event(
        stage="egress",
        event="codexproxy.api.response.cleanup_failed",
        source="api",
        operation=operation,
...
async def _close_pre_start_body(
    ...
    *,
    ...
    task = asyncio.create_task(
        ...
        name="cdx-api-pre-start-stream-cleanup",
    )
    await _wait_for_cleanup(task)
