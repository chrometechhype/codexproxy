...
class ProviderExecutor:
    ...
    def _progress_timeout_failure(
        ...
        *,
        ...
        trace_event(
            stage="execution",
            event="codexproxy.provider.progress_timeout",
            source="application",
    ...
    def stream(
        ...
        *,
        ...
        trace_event(
            stage="ingress",
            event=(
                "codexproxy.api.responses.request.received"
                if wire_api == "responses"
                else "codexproxy.api.request.received"
            ),
            source="api",
        ...
        return traced_async_stream(
            ...
            source="api",
            complete_event=(
                "codexproxy.api.responses.stream_completed"
                if wire_api == "responses"
                else "codexproxy.api.response.stream_completed"
            ),
            interrupted_event=(
                "codexproxy.api.responses.stream_interrupted"
                if wire_api == "responses"
                else "codexproxy.api.response.stream_interrupted"
            ),
            chunk_event=None,
            extra=stream_trace,
        )
