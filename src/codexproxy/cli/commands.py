"""Implementations for installed CodexProxy commands."""
...
from codexproxy.cli.process_registry import kill_all_best_effort
...
from codexproxy.runtime.bootstrap import build_asgi_app
...
class ServerSupervisor:
    """Own one CDX server lifecycle, including config-driven restarts."""
    ...
    def run(self, *, open_admin_browser: bool | None = None) -> None:
        ...
        with self._lock:
            self._run_scheduled = False
            if self._running:
                raise RuntimeError("The CDX server supervisor is already running.")
