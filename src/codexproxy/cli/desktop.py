"""Platform-neutral lifecycle for the CDX desktop shell."""

import threading
from collections.abc import Callable
from typing import Protocol

from codexproxy.cli.commands import (
    ServerStatus,
    ServerSupervisor,
...
from codexproxy.config.paths import config_dir_path
from codexproxy.config.server_urls import local_proxy_root_url
from codexproxy.core.interprocess_lock import InterprocessFileLock
...
class DesktopController:
    """Coordinate one tray loop with one in-process CDX server owner."""
    ...
    def run(self) -> None:
        """Run the tray on this thread and the CDX server on its owned worker."""
...
def launch_desktop(tray_factory: DesktopTrayFactory) -> None:
    """Start the singleton desktop host or focus the already running CDX UI."""
