"""Process-local Cline CLI configuration for CDX model routing."""
...
# Cline's released session gateway only instantiates built-in providers. CDX
...
def build_cline_config(
    ...
    *,
    ...
    """Translate a non-empty CDX model snapshot into Cline's file contracts."""

    if not models:
        raise ValueError("Cline requires at least one routable CDX model")
