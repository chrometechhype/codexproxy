...
from codexproxy.config.paths import codex_model_catalog_path
...
def build_codex_launcher_command(
    *,
    ...
    """Return a Codex command with ephemeral CDX provider config."""
...
def codex_config_args(*, api_url: str, model: str | None = None) -> list[str]:
    """Return Codex `-c` assignments for the ephemeral CDX provider."""

    args = [
        "-c",
        _toml_assignment("model_provider", "cdx"),
        "-c",
        _toml_assignment("model_providers.cdx.name", "CodexProxy"),
        "-c",
        _toml_assignment("model_providers.cdx.base_url", _ensure_v1_url(api_url)),
        "-c",
        _toml_assignment("model_providers.cdx.auth.command", "cdx-codex"),
        "-c",
        _toml_assignment(
            "model_providers.cdx.auth.args", [_PRINT_PROXY_AUTH_TOKEN_FLAG]
        ),
        "-c",
        _toml_assignment("model_providers.cdx.wire_api", "responses"),
    ]
    if model:
