"""Shared CDX model-catalog projection for installed client launchers."""
...
from codexproxy.cli.local_http import open_local_request
from codexproxy.config.provider_catalog import SUPPORTED_PROVIDER_IDS
from codexproxy.core.gateway_model_ids import (
    GATEWAY_MODEL_ID_PREFIX,
    NO_THINKING_GATEWAY_MODEL_ID_PREFIX,
)
from codexproxy.core.json_types import JsonObject, JsonValue
...
class ClientModel:
    """One direct CDX model reference suitable for a client-side catalog."""
...
def client_models_from_response(
    ...
    """Project an CDX `/v1/models` response into direct client model records."""
...
def fetch_proxy_models_response(proxy_root_url: str, auth_token: str) -> JsonObject:
    """Fetch the authenticated CDX-local `/v1/models` response directly."""
