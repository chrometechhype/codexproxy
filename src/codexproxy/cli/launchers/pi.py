...
def launch(argv: Sequence[str] | None = None) -> None:
    ...
    if not extension_path.is_file():
        print(
            "CodexProxy's bundled Pi extension is missing. Reinstall CDX.",
            file=sys.stderr,
        )
        raise SystemExit(1)
...
def build_pi_launcher_command(
    *,
    ...
    """Return a Pi session command with ephemeral CDX provider registration."""
...
def pi_binary_is_compatible(binary_path: str) -> bool:
    """Return whether an executable exposes the Pi features CDX requires."""
