...
const NO_THINKING_MODEL_PREFIX = "claude-3-codexcc-no-thinking/";
...
function catalogModelIds(payload: unknown): string[] {
	if (!isRecord(payload) || payload.object !== "list" || !Array.isArray(payload.data)) {
		throw new Error("CDX model catalog returned an invalid response shape.");
	}
...
export function projectFccModels(payload: unknown): ProviderModelConfig[] {
	...
	if (models.length === 0) {
		throw new Error("CDX model catalog contains no routable provider models.");
	}
	return models;
}
	...
	try {
		...
		try {
			...
			if (error instanceof Error && error.name === "AbortError") {
				throw new Error(`CDX model catalog timed out after ${CATALOG_TIMEOUT_MS}ms.`);
			}
			const message = error instanceof Error ? error.message : String(error);
			throw new Error(`Could not reach the CDX model catalog: ${message}`);
		}

		if (!response.ok) {
			throw new Error(`CDX model catalog returned HTTP ${response.status}${requestIdSuffix(response)}.`);
		}
		...
		try {
			...
			throw new Error(`CDX model catalog returned invalid JSON${requestIdSuffix(response)}.`);
		}
...
export default async function CodexProxy(pi: ExtensionAPI): Promise<void> {
