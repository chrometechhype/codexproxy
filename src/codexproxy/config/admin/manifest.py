...
from codexproxy.config.reasoning import (
...
_NON_PROVIDER_FIELDS: tuple[ConfigFieldSpec, ...] = (
    ...
    ConfigFieldSpec(
        ...
        description="Require the retained API/CLI token on CDX API routes.",
    ),
    ConfigFieldSpec(
...
def _catalog_smoke_fields() -> tuple[ConfigFieldSpec, ...]:
    return tuple(
        ConfigFieldSpec(
            key=f"CODEX_PROXY_SMOKE_MODEL_{provider_id.upper()}",
