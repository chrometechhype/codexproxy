"""One-time consolidation of legacy CDX dotenv state."""
...
LEGACY_OPENCODE_SMOKE_MODEL_ENV = "CODEX_PROXY_SMOKE_MODEL_OPENCODE"
OPENCODE_ZEN_SMOKE_MODEL_ENV = "CODEX_PROXY_SMOKE_MODEL_OPENCODE_ZEN"
...
def recognized_env_keys() -> frozenset[str]:
    """Return every dotenv key CDX may import into managed configuration."""
...
def consolidate_managed_config(
    ...
    """Consolidate pre-schema configuration into ``~/.cdx/.env`` once."""
    ...
    for key in _RETIRED_ENV_KEYS:
        values.pop(key, None)
    values.pop(CODEX_PROXY_CONFIG_SCHEMA_ENV, None)
    _materialize_auth_state(values, process, had_legacy_state=base_path is not None)
    for key in tuple(values):
        if (
            key in known
            and key != CODEX_PROXY_CONFIG_SCHEMA_ENV
            and not values[key].strip()
        ):
            values.pop(key)
    values[CODEX_PROXY_CONFIG_SCHEMA_ENV] = CONFIG_SCHEMA_VERSION
...
def render_managed_config(
    ...
    *,
    ...
    """Render sparse canonical CDX configuration."""

    known = recognized_env_keys()
    secret_keys = secret_env_keys()
    canonical = dict(values)
    canonical[CODEX_PROXY_CONFIG_SCHEMA_ENV] = CONFIG_SCHEMA_VERSION
    lines = [
        "# Managed by CodexProxy.",
        "# Edit settings in /admin when possible.",
        f"{CODEX_PROXY_CONFIG_SCHEMA_ENV}={CONFIG_SCHEMA_VERSION}",
    ]
    configured = [
        key
        for key in canonical
        if key != CODEX_PROXY_CONFIG_SCHEMA_ENV
        and key in known
        and canonical[key].strip()
    ]
    if configured:
