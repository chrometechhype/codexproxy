"""Contract tests for the installed `cdx-cline` launcher."""
...
from codexproxy.cli.launchers.cline_config import (
    CLINE_PROVIDER_ID,
    build_cline_config,
)
...
def test_cline_management_commands_are_native_passthrough(argv: list[str]) -> None:
    from codexproxy.cli.launchers import cline
...
def test_cline_rejects_sandbox_overrides_that_replace_ephemeral_credentials(
    ...
    assert exc_info.value.code == 2
    assert "replaces the ephemeral CDX provider file" in capsys.readouterr().err
    require_version.assert_not_called()
    run_client_process.assert_not_called()
...
def test_cline_root_positional_scanner(argv: list[str], expected: str | None) -> None:
    from codexproxy.cli.launchers.cline import first_root_positional
...
def test_cline_child_env_is_process_local_and_preserves_native_state(
    ...
    from codexproxy.cli.launchers.cline import build_cline_launcher_env
...
def test_cline_fails_closed_when_proxy_is_unreachable(
    ...
    assert "cdx-server" in capsys.readouterr().err
