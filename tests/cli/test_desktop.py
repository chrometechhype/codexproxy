...
from codexproxy.cli.commands import ServerStatus, ServerSupervisor
from codexproxy.cli.desktop import DesktopController
...
def test_supervisor_accepts_restart_during_scheduled_startup() -> None:
    ...
    with (
        patch(
            "codexproxy.cli.commands.load_server_settings",
            return_value=settings,
        ),
        patch.object(supervisor, "_run_once", return_value=False) as run_once,
        patch("codexproxy.cli.commands.kill_all_best_effort"),
    ):
...
def test_second_desktop_launch_opens_existing_admin_without_new_server() -> None:
    from codexproxy.cli import desktop
