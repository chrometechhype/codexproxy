...
def test_legacy_env_migration_supports_xdg_path(tmp_path: Path) -> None:
    """Server startup preserves config from ~/.config/codexproxy/.env."""
    from cli.commands import _migrate_legacy_env_if_missing
...
def test_cli_scripts_are_registered() -> None:
    ...
    assert pyproject["project"]["scripts"] == {
        "cdx-server": "cli.entrypoints:serve",
        "codexproxy": "cli.entrypoints:serve",
        "cdx-codex": "cli.launchers.codex:launch",
        "cdx-claude": "cli.launchers.claude:launch",
        "cdx-pi": "cli.launchers.pi:launch",
        "cdx-desktop": "cli.desktop_entrypoint:launch",
    }
...
def test_CODEX_PROXY_server_reports_version_without_side_effects(
    argv: tuple[str, ...],
...
def test_non_version_entrypoint_delegates_to_server_command() -> None:
    from cli import commands, entrypoints
...
def test_serve_migrates_legacy_env_before_loading_settings(tmp_path: Path) -> None:
    ...
    assert (tmp_path / ".codexproxy" / ".env").read_text("utf-8") == (
...
def test_config_env_key_migration_warns_for_explicit_env_file(
    ...
    with patch.dict(commands.os.environ, {"CODEX_PROXY_ENV_FILE": str(explicit)}):
...
def test_launch_claude_passes_args_and_child_env(
    ...
    with (
        ...
        patch(
            "cli.launchers.common.shutil.which",
...
def test_launch_codex_passes_responses_config_and_child_env(
    ...
    with (
        ...
        patch(
            "cli.launchers.codex.open_local_request",
    ...
    assert 'model_provider="codexproxy"' in command
    assert 'model_providers.codexproxy.base_url="http://127.0.0.1:9191/v1"' in command
    assert 'model_providers.codexproxy.wire_api="responses"' in command
    ...
    child_env = popen.call_args.kwargs["env"]
    assert child_env["CODEX_PROXY_CODEX_API_KEY"] == "proxy-token"
...
def test_pi_launcher_uses_no_auth_sentinel_for_blank_token() -> None:
    from cli.launchers.pi import build_pi_launcher_env
    ...
    assert env["CODEX_PROXY_PI_API_KEY"] == "cdx-no-auth"
...
def test_launch_pi_registers_bundled_extension_for_sessions(
    ...
    from cli.launchers.pi import launch
    ...
    with (
        patch("cli.launchers.pi.get_settings", return_value=settings),
        patch("cli.launchers.pi.preflight_proxy", return_value=None),
        patch(
            "cli.launchers.pi.pi_extension_path",
        ...
        patch(
            "cli.launchers.pi.pi_binary_is_compatible",
            return_value=True,
        ),
        patch("cli.launchers.common.subprocess.Popen") as popen,
        patch("cli.launchers.common.register_pid"),
...
def test_launch_pi_passes_management_commands_through_without_proxy(
    ...
    with (
        patch("cli.launchers.pi.get_settings") as get_settings,
        patch("cli.launchers.pi.preflight_proxy") as preflight,
        patch(
            "cli.launchers.common.shutil.which",
...
def test_pi_install_hints_use_official_platform_installers() -> None:
    from cli.launchers.pi import pi_install_hint
...
def test_pi_binary_compatibility_requires_both_launcher_capabilities(
    ...
    from cli.launchers.pi import pi_binary_is_compatible

    with patch(
        "cli.launchers.pi.subprocess.run",
...
def test_launch_claude_unreachable_proxy_exits_with_hint(
    ...
    with (
        patch("cli.launchers.claude.get_settings", return_value=settings),
        patch(
            "cli.launchers.claude.preflight_proxy",
