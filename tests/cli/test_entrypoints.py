...
def test_cli_scripts_are_registered() -> None:
    ...
    assert pyproject["project"]["scripts"] == {
        ...
        "cdx-claude": "codexproxy.cli.launchers.claude:launch",
        "cdx-codex": "codexproxy.cli.launchers.codex:launch",
        "cdx-pi": "codexproxy.cli.launchers.pi:launch",
        "cdx-opencode": "codexproxy.cli.launchers.opencode:launch",
...
def test_version_entrypoint_does_not_import_command_runtime() -> None:
    script = "\n".join(
        (
            "import json",
            "import sys",
            "from codexproxy.cli.entrypoints import serve",
            "serve(['--version'])",
...
def test_non_version_entrypoint_delegates_to_server_command() -> None:
    from codexproxy.cli import commands, entrypoints
...
def test_schedule_open_admin_browser_opens_when_health_ready() -> None:
    ...
    from codexproxy.cli import commands
...
def test_launch_claude_passes_args_and_child_env(
    ...
    from codexproxy.cli.launchers.claude import launch
    ...
    with (
        patch("codexproxy.cli.launchers.claude.get_settings", return_value=settings),
        patch("codexproxy.cli.launchers.claude.preflight_proxy", return_value=None),
        patch(
            "codexproxy.cli.launchers.common.shutil.which",
            return_value="resolved-claude.cmd",
        ),
        patch("codexproxy.cli.launchers.common.subprocess.Popen") as popen,
        patch("codexproxy.cli.launchers.common.register_pid") as register_pid,
        patch("codexproxy.cli.launchers.common.unregister_pid") as unregister_pid,
...
def test_launch_codex_passes_responses_config_and_child_env(
    ...
    from codexproxy.cli.launchers.codex import launch
    ...
    with (
        patch("codexproxy.cli.launchers.codex.get_settings", return_value=settings),
        patch("codexproxy.cli.launchers.codex.preflight_proxy", return_value=None),
        patch(
            "codexproxy.cli.launchers.common.shutil.which",
            return_value="resolved-codex.cmd",
        ),
        patch(
            "codexproxy.cli.launchers.codex.codex_model_catalog_path",
            return_value=catalog_path,
        ),
        patch(
            "codexproxy.cli.launchers.model_catalog.open_local_request",
    ...
    assert 'model_provider="cdx"' in command
    assert 'model_providers.cdx.base_url="http://127.0.0.1:9191/v1"' in command
    assert 'model_providers.cdx.auth.command="cdx-codex"' in command
    assert 'model_providers.cdx.auth.args=["--print-proxy-auth-token"]' in command
    assert 'model_providers.cdx.wire_api="responses"' in command
    assert not any("model_providers.cdx.env_key" in arg for arg in command)
    ...
    child_env = popen.call_args.kwargs["env"]
    assert "CODEX_PROXY_CODEX_API_KEY" not in child_env
...
def test_codex_proxy_auth_command_prints_only_current_token(
    ...
    from codexproxy.cli.launchers import codex
...
def test_launch_codex_catalog_failure_warns_and_continues(
    ...
    with (
        ...
        patch("codexproxy.cli.launchers.common.subprocess.Popen") as popen,
        patch("codexproxy.cli.launchers.common.register_pid"),
        patch("codexproxy.cli.launchers.common.unregister_pid"),
        pytest.raises(SystemExit) as exc_info,
    ):
        process = popen.return_value
        process.pid = 12345
...
def test_pi_launcher_builds_scoped_session_command_and_proxy_env(
    ...
    from codexproxy.cli.launchers.pi import (
    ...
    env = build_pi_launcher_env(
        ...
        base_env={
            ...
            "CODEX_PROXY_PI_API_KEY": "stale-key",
            "CODEX_PROXY_PI_BASE_URL": "https://stale.invalid",
        },
    )
    ...
    assert env == {
        ...
        "no_proxy": "127.0.0.1,localhost,::1",
        "CODEX_PROXY_PI_BASE_URL": "http://127.0.0.1:9191",
        "CODEX_PROXY_PI_API_KEY": "proxy-token",
    }
...
def test_launch_pi_registers_bundled_extension_for_sessions(
    ...
    from codexproxy.cli.launchers.pi import launch

    monkeypatch.setenv("KEEP_ME", "yes")
    monkeypatch.setenv("CODEX_PROXY_PI_API_KEY", "stale-key")
    extension = tmp_path / "pi_extension.ts"
    extension.write_text("export default () => {};", encoding="utf-8")
    settings = _launcher_settings(port=9191, token="proxy-token")

    with (
        patch("codexproxy.cli.launchers.pi.get_settings", return_value=settings),
        patch("codexproxy.cli.launchers.pi.preflight_proxy", return_value=None),
        patch(
            "codexproxy.cli.launchers.pi.pi_extension_path",
        ...
        patch(
            "codexproxy.cli.launchers.pi.pi_binary_is_compatible",
            return_value=True,
        ),
        patch("codexproxy.cli.launchers.common.subprocess.Popen") as popen,
        patch("codexproxy.cli.launchers.common.register_pid"),
    ...
    child_env = popen.call_args.kwargs["env"]
    assert child_env["CODEX_PROXY_PI_BASE_URL"] == "http://127.0.0.1:9191"
    assert child_env["CODEX_PROXY_PI_API_KEY"] == "proxy-token"
    assert child_env["NO_PROXY"] == "127.0.0.1,localhost,::1"
    assert child_env["no_proxy"] == child_env["NO_PROXY"]
    assert child_env["KEEP_ME"] == "yes"
...
def test_launch_pi_passes_management_commands_through_without_proxy(
    ...
    with (
        patch("codexproxy.cli.launchers.pi.get_settings") as get_settings,
        patch("codexproxy.cli.launchers.pi.preflight_proxy") as preflight,
        patch(
            "codexproxy.cli.launchers.common.shutil.which",
...
def test_pi_install_hints_use_official_platform_installers() -> None:
    from codexproxy.cli.launchers.pi import pi_install_hint
...
def test_pi_binary_compatibility_requires_both_launcher_capabilities(
    ...
    from codexproxy.cli.launchers.pi import pi_binary_is_compatible

    with patch(
        "codexproxy.cli.launchers.pi.subprocess.run",
...
def test_launch_claude_keyboard_interrupt_kills_child_tree() -> None:
    ...
    with (
        ...
        patch("codexproxy.cli.launchers.common.subprocess.Popen") as popen,
        patch("codexproxy.cli.launchers.common.register_pid"),
        patch("codexproxy.cli.launchers.common.kill_pid_tree_best_effort") as kill_tree,
        patch("codexproxy.cli.launchers.common.unregister_pid") as unregister_pid,
...
def test_launch_claude_exits_when_command_cannot_be_resolved(
    ...
    with (
        patch("codexproxy.cli.launchers.claude.get_settings", return_value=settings),
        patch("codexproxy.cli.launchers.claude.preflight_proxy", return_value=None),
        patch("codexproxy.cli.launchers.common.shutil.which", return_value=None),
        patch("codexproxy.cli.launchers.common.subprocess.Popen") as popen,
        pytest.raises(SystemExit) as exc_info,
    ):
        launch([])
...
def test_launch_claude_unreachable_proxy_exits_with_hint(
    ...
    with (
        patch("codexproxy.cli.launchers.claude.get_settings", return_value=settings),
        patch(
            "codexproxy.cli.launchers.claude.preflight_proxy",
    ...
    assert "cdx-server" in captured.err
