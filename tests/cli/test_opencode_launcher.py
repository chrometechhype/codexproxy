...
from codexproxy.cli.launchers.model_catalog import ClientModel
from codexproxy.cli.launchers.opencode_config import build_opencode_config
...
def _models_payload() -> JsonObject:
    return {
        "data": [
            ...
            {
                "id": "claude-3-codexcc-no-thinking/open_router/plain-model",
...
def test_opencode_config_uses_responses_sdk_and_only_known_metadata() -> None:
    config = build_opencode_config(
        (
            ...
            ClientModel(
                wire_slug="claude-3-codexcc-no-thinking/open_router/plain-model",
    ...
    cdx = provider["codexproxy"]
    assert isinstance(cdx, dict)
    assert cdx["npm"] == "@ai-sdk/openai"
    assert cdx["options"] == {
        "baseURL": "http://127.0.0.1:9191/v1",
        "apiKey": "{env:CODEX_PROXY_OPENCODE_API_KEY}",
    }
    assert cdx["models"] == {
        ...
        "claude-3-codexcc-no-thinking/open_router/plain-model": {
    ...
    assert config.overlay == {
        ...
        "enabled_providers": ["codexproxy"],
        "disabled_providers": [],
        "model": "codexproxy/nvidia_nim/vendor/model",
        "small_model": "codexproxy/nvidia_nim/vendor/model",
    }
...
def test_opencode_child_env_is_process_local_and_preserves_user_state(
    ...
    from codexproxy.cli.launchers.opencode import build_opencode_launcher_env
    ...
    env = build_opencode_launcher_env(
        ...
        overlay={"model": "codexproxy/nvidia_nim/vendor/model"},
        base_env={
            "PATH": "keep",
            "OPENCODE_CONFIG": "",
            "OPENCODE_CONFIG_CONTENT": "",
            "OPENCODE_CONFIG_DIR": "keep-user-state",
            "CODEX_PROXY_OPENCODE_API_KEY": "stale-token",
            "CODEX_PROXY_OPENCODE_OTHER": "stale-value",
    ...
    assert env["CODEX_PROXY_OPENCODE_API_KEY"] == "proxy-token"
    assert "CODEX_PROXY_OPENCODE_OTHER" not in env
...
def test_opencode_version_probe(
    ...
    from codexproxy.cli.launchers.opencode import opencode_binary_version

    with patch(
        "codexproxy.cli.launchers.opencode.subprocess.run",
...
def test_opencode_management_commands_pass_through_without_fcc(
    ...
    from codexproxy.cli.launchers import opencode
...
def test_opencode_inference_commands_are_never_raw_passthrough(
    ...
    from codexproxy.cli.launchers.opencode import is_opencode_passthrough
...
def test_opencode_models_is_fcc_configured_and_temp_config_is_cleaned(
    ...
    assert "codexproxy" in providers
    assert observed_env is not None
    assert observed_env["CODEX_PROXY_OPENCODE_API_KEY"] == "proxy-token"
...
def test_opencode_fails_closed_when_catalog_has_no_routable_models(
    ...
    assert exc_info.value.code == 1
    run_client_process.assert_not_called()
    assert "at least one routable CDX model" in capsys.readouterr().err
