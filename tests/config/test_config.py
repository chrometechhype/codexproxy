...
from codexproxy.config.constants import (
    ANTHROPIC_DEFAULT_MAX_OUTPUT_TOKENS,
    HTTP_CONNECT_TIMEOUT_DEFAULT,
)
from codexproxy.config.loader import (
    ConfigSource,
    clear_settings_cache,
    compose_settings_snapshot,
    get_settings,
)
from codexproxy.config.model_refs import (
    configured_chat_model_refs,
    parse_model_name,
    parse_provider_type,
)
from codexproxy.config.nim import NimSettings
from codexproxy.config.paths import messaging_state_dir_path, server_log_path
...
def test_settings_defaults_are_valid_and_nonempty() -> None:
    ...
    assert settings.proxy_auth_token == "codexcc"
    assert [
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("CODEX_PROXY_OPEN_BROWSER", "open_admin_browser", "false", False),
...
def test_blank_process_auth_token_uses_retained_default() -> None:
    ...
    assert snapshot.settings.proxy_auth_token == "codexcc"
...
def test_paths_are_owned_by_fcc_home(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    assert messaging_state_dir_path() == tmp_path / ".cdx" / "agent_workspace"
    assert server_log_path() == tmp_path / ".cdx" / "logs" / "server.log"
