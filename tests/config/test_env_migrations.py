...
from codexproxy.config import env_migrations
from codexproxy.config.env_files import dotenv_values_from_file
from codexproxy.config.env_migrations import (
...
from codexproxy.config.loader import resolve_settings_snapshot


def _paths(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> tuple[Path, Path]:
    managed = tmp_path / ".cdx" / ".env"
...
def test_fresh_install_writes_only_schema_metadata(
    ...
    assert dotenv_values_from_file(managed) == {
        "CODEX_PROXY_CONFIG_SCHEMA": CONFIG_SCHEMA_VERSION
    }
...
def test_explicit_file_overlays_once_and_then_becomes_inert(
    ...
    consolidate_managed_config({"CODEX_PROXY_ENV_FILE": str(explicit)})
    ...
    result = consolidate_managed_config({"CODEX_PROXY_ENV_FILE": str(explicit)})
...
def test_post_schema_invalid_explicit_path_is_never_dereferenced(
    ...
    managed, _legacy = _paths(monkeypatch, tmp_path)
    managed.parent.mkdir(parents=True)
    managed.write_text("CODEX_PROXY_CONFIG_SCHEMA=1\n", encoding="utf-8")

    result = consolidate_managed_config(
        {"CODEX_PROXY_ENV_FILE": str(tmp_path / "missing.env")}
    )
...
def test_explicit_managed_path_is_deduplicated(
    ...
    result = consolidate_managed_config({"CODEX_PROXY_ENV_FILE": str(managed)})
...
def test_only_managed_schema_marker_is_trusted(
    ...
    managed, legacy = _paths(monkeypatch, tmp_path)
    legacy.parent.mkdir(parents=True)
    legacy.write_text("CODEX_PROXY_CONFIG_SCHEMA=99\nMODEL=deepseek/imported\n")

    consolidate_managed_config({"CODEX_PROXY_CONFIG_SCHEMA": "99"})

    values = dotenv_values_from_file(managed)
    assert values["CODEX_PROXY_CONFIG_SCHEMA"] == "1"
...
def test_concurrent_loaders_produce_one_valid_managed_file(
    ...
    assert dotenv_values_from_file(managed)["CODEX_PROXY_CONFIG_SCHEMA"] == "1"
...
def test_newer_managed_schema_fails_closed(
    ...
    managed, _legacy = _paths(monkeypatch, tmp_path)
    managed.parent.mkdir(parents=True)
    managed.write_text("CODEX_PROXY_CONFIG_SCHEMA=2\n", encoding="utf-8")
