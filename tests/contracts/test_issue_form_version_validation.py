...
def test_bug_form_requests_a_contained_version_or_none() -> None:
    ...
    assert "Run `cdx-server --version`" in form
...
def test_workflow_owns_one_idempotent_triage_state() -> None:
    ...
    assert "needs-cdx-version" in workflow
    assert "<!-- cdx-version-validator -->" in workflow
...
await run(github, context);
await run(github, context);
liveIssue.body = bodyFor("codexproxy 17.23.455");
await run(github, context);
...
comments.push({
  id: 102,
  user: { login: "github-actions[bot]" },
  body: "<!-- cdx-version-outdated -->\nstale",
});
liveIssue.labels = [{ name: "needs-cdx-version" }];
...
"""
    ...
    assert "`17.23.454`" in next(
        ...
        and "cdx-version-outdated" in call["args"]["body"]
    )
