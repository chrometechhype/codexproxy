...
def test_ollama_cloud_provider_configuration_uses_api_key(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_OLLAMA_CLOUD", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_provider_smoke_models_cover_configured_providers_independent_of_model_mapping(
    ...
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_DEEPSEEK", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
...
def test_xai_provider_smoke_uses_current_grok_model(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_XAI", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_cline_pass_provider_smoke_uses_low_cost_subscription_model(
    ...
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_CLINE_PASS", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_cline_pass_smoke_override_accepts_full_or_upstream_model_ref(
    ...
    for override in (
        ...
        monkeypatch.setenv("CODEX_PROXY_SMOKE_MODEL_CLINE_PASS", override)
        models = _smoke_config(settings=settings).provider_smoke_models()

        assert [model.provider for model in models] == ["cline_pass"]
        assert models[0].full_model == "cline_pass/cline-pass/kimi-k3"
        assert models[0].source == "CODEX_PROXY_SMOKE_MODEL_CLINE_PASS"


def test_zai_shared_key_enables_both_provider_smoke_surfaces(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_ZAI", raising=False)
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_ZAI_API", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_zai_api_smoke_override_is_independent_from_coding_plan(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_ZAI", raising=False)
    monkeypatch.setenv("CODEX_PROXY_SMOKE_MODEL_ZAI_API", "zai_api/glm-5.2")
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
    ...
    assert [(model.provider, model.full_model, model.source) for model in models] == [
        ("zai", "zai/glm-5.2", "provider_default"),
        ("zai_api", "zai_api/glm-5.2", "CODEX_PROXY_SMOKE_MODEL_ZAI_API"),
    ]


def test_qwencloud_provider_smoke_uses_current_coding_model(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_QWENCLOUD", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_qwencloud_coding_provider_smoke_uses_recommended_model(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_QWENCLOUD_CODING", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_qwencloud_coding_smoke_override_is_independent_from_token_plan(
    ...
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_QWENCLOUD", raising=False)
    monkeypatch.setenv(
        "CODEX_PROXY_SMOKE_MODEL_QWENCLOUD_CODING",
...
def test_together_provider_smoke_uses_current_coding_model(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_TOGETHER", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_deepinfra_provider_smoke_uses_current_coding_model(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_DEEPINFRA", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_siliconflow_provider_smoke_uses_documented_chat_model(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_SILICONFLOW", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_nebius_provider_smoke_uses_documented_agent_model(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_NEBIUS", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_chutes_provider_smoke_uses_documented_agent_model(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_CHUTES", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_featherless_provider_smoke_uses_documented_agent_model(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_FEATHERLESS", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_agnes_provider_smoke_uses_documented_model(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_AGNES", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_zenmux_provider_smoke_uses_current_free_model(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_ZENMUX", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_wandb_provider_smoke_uses_documented_tool_model(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_WANDB", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_wandb_provider_smoke_honors_model_override(monkeypatch) -> None:
    monkeypatch.setenv("CODEX_PROXY_SMOKE_MODEL_WANDB", "deepseek-ai/DeepSeek-V4-Flash")
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
    ...
    assert models[0].source == "CODEX_PROXY_SMOKE_MODEL_WANDB"


def test_connected_account_provider_smoke_requires_explicit_model(
    monkeypatch,
) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_OPENAI", raising=False)
    config = _smoke_config(
    ...
    monkeypatch.setenv("CODEX_PROXY_SMOKE_MODEL_OPENAI", "gpt-5.3-codex")
    ...
    assert config.provider_smoke_models() == [
        ProviderModel(
            ...
            source="CODEX_PROXY_SMOKE_MODEL_OPENAI",
        )
    ]


def test_openrouter_provider_smoke_uses_concrete_free_model(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_OPEN_ROUTER", raising=False)
    config = _smoke_config(
...
def test_kilo_provider_smoke_uses_concrete_free_model(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_KILO", raising=False)
    config = _smoke_config(
...
def test_bedrock_provider_configuration_uses_official_api_key(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_BEDROCK", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_azure_openai_provider_configuration_requires_key_and_resource_url(
    ...
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_AZURE_OPENAI", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_vertex_provider_configuration_uses_project_id(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_VERTEX", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_wafer_provider_configuration_uses_api_key(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_WAFER", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_kimi_code_provider_configuration_uses_subscription_key(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_KIMI_CODE", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_minimax_provider_configuration_uses_api_key(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_MINIMAX", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_cloudflare_provider_configuration_requires_token_and_account(
    ...
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_CLOUDFLARE", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_vercel_provider_configuration_uses_api_key(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_VERCEL", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_huggingface_provider_configuration_uses_api_key(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_HUGGINGFACE", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_cohere_provider_configuration_uses_api_key(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_COHERE", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_github_models_provider_configuration_uses_token(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_GITHUB_MODELS", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_sambanova_provider_configuration_uses_api_key(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_SAMBANOVA", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_provider_smoke_model_override_accepts_model_name_without_prefix(
    ...
    monkeypatch.setenv("CODEX_PROXY_SMOKE_MODEL_DEEPSEEK", "deepseek-reasoner")
    config = _smoke_config(
        settings=_settings(
            deepseek_api_key="deepseek-key",
            ollama_base_url="",
    ...
    assert models[0].source == "CODEX_PROXY_SMOKE_MODEL_DEEPSEEK"
...
def test_provider_smoke_model_override_accepts_owner_model_name(
    ...
    monkeypatch.setenv(
        "CODEX_PROXY_SMOKE_MODEL_NVIDIA_NIM", "nvidia/nemotron-3-super-120b-a12b"
    )
    config = _smoke_config(
        settings=_settings(
    ...
    assert models[0].source == "CODEX_PROXY_SMOKE_MODEL_NVIDIA_NIM"


def test_provider_smoke_model_override_preserves_namespaced_upstream_model(
    monkeypatch,
) -> None:
    monkeypatch.setenv("CODEX_PROXY_SMOKE_MODEL_DEEPSEEK", "ollama/llama3.1")
    config = _smoke_config(
        settings=_settings(
            deepseek_api_key="deepseek-key",
            ollama_base_url="",
...
def test_mistral_reasoning_smoke_uses_reasoning_default(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_MISTRAL_REASONING", raising=False)
    config = _smoke_config(
...
def test_mistral_reasoning_smoke_accepts_override(monkeypatch) -> None:
    monkeypatch.setenv(
        "CODEX_PROXY_SMOKE_MODEL_MISTRAL_REASONING", "mistral-medium-3-5"
    )
    config = _smoke_config(
        settings=_settings(mistral_api_key="mistral-key", ollama_base_url="")
    )
    ...
    assert model.source == "CODEX_PROXY_SMOKE_MODEL_MISTRAL_REASONING"
...
def test_provider_smoke_collection_params_are_grouped_by_provider(
    ...
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_DEEPSEEK", raising=False)
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_NVIDIA_NIM", raising=False)
    config = _smoke_config(
        live=True,
        settings=_settings(
            deepseek_api_key="deepseek-key",
...
def test_provider_smoke_includes_local_provider_when_model_mapping_uses_it(
    ...
    monkeypatch.delenv("CODEX_PROXY_SMOKE_MODEL_OLLAMA", raising=False)
    config = _smoke_config()
...
def test_nvidia_nim_cli_models_override_and_append() -> None:
    refs = nvidia_nim_cli_model_refs(
        {
            "CODEX_PROXY_SMOKE_NIM_MODELS": "z-ai/glm-5.2,nvidia_nim/custom/model",
            "CODEX_PROXY_SMOKE_NIM_EXTRA_MODELS": "moonshotai/kimi-k2.6,z-ai/glm-5.2",
        }
    )
    ...
    assert refs["nvidia_nim/z-ai/glm-5.2"] == "CODEX_PROXY_SMOKE_NIM_MODELS"
    assert refs["nvidia_nim/moonshotai/kimi-k2.6"] == (
        "CODEX_PROXY_SMOKE_NIM_EXTRA_MODELS"
    )


def test_nvidia_nim_cli_models_reject_empty_override() -> None:
    try:
        nvidia_nim_cli_model_refs({"CODEX_PROXY_SMOKE_NIM_MODELS": " , "})
    except ValueError as exc:
        assert "CODEX_PROXY_SMOKE_NIM_MODELS" in str(exc)
    else:
...
def test_nvidia_nim_cli_models_preserve_namespaced_upstream_model() -> None:
    refs = nvidia_nim_cli_model_refs(
        {"CODEX_PROXY_SMOKE_NIM_MODELS": "open_router/model"}
    )

    assert refs == {
        "nvidia_nim/open_router/model": "CODEX_PROXY_SMOKE_NIM_MODELS",
    }


def test_smoke_config_returns_nvidia_nim_cli_provider_models(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_NIM_MODELS", raising=False)
    monkeypatch.delenv("CODEX_PROXY_SMOKE_NIM_EXTRA_MODELS", raising=False)
    config = _smoke_config(
        settings=_settings(
    ...
    refs = openrouter_free_cli_model_refs(
        {
            ...
            "CODEX_PROXY_SMOKE_OPENROUTER_FREE_EXTRA_MODELS": (
...
def test_openrouter_free_cli_models_reject_empty_override() -> None:
    try:
        ...
        assert "CODEX_PROXY_SMOKE_OPENROUTER_FREE_MODELS" in str(exc)
    else:
...
def test_openrouter_free_cli_models_preserve_namespaced_upstream_model() -> None:
    refs = openrouter_free_cli_model_refs(
        {"CODEX_PROXY_SMOKE_OPENROUTER_FREE_MODELS": "nvidia_nim/model"}
    )

    assert refs == {
        "open_router/nvidia_nim/model": "CODEX_PROXY_SMOKE_OPENROUTER_FREE_MODELS",
    }


def test_smoke_config_returns_openrouter_free_cli_provider_models(monkeypatch) -> None:
    monkeypatch.delenv("CODEX_PROXY_SMOKE_OPENROUTER_FREE_MODELS", raising=False)
    monkeypatch.delenv("CODEX_PROXY_SMOKE_OPENROUTER_FREE_EXTRA_MODELS", raising=False)
    config = _smoke_config(
        settings=_settings(
