...
async def test_registry_validation_succeeds_for_all_configured_models() -> None:
    ...
    settings = _settings(model="nvidia_nim/nim-model")
...
async def test_registry_validation_reports_missing_model_with_sources() -> None:
    ...
    assert "sources=MODEL" in message
...
@pytest.mark.asyncio
async def test_registry_validation_aggregates_single_failure() -> None:
    """A single missing model surfaces a 'missing model' problem line."""
    registry = ProviderRegistry(
        {
            "nvidia_nim": FakeProvider(frozenset({"different-model"})),
        }
...
@pytest.mark.asyncio
async def test_registry_validation_reports_malformed_response() -> None:
    """A provider returning a bad model-list shape surfaces a malformed problem."""
    registry = ProviderRegistry(
        {
...
@pytest.mark.asyncio
async def test_registry_validation_completes_within_timeout() -> None:
    """Single configured MODEL completes the validation pass within a tight bound."""
    registry = ProviderRegistry(
        {
            "nvidia_nim": FakeProvider(frozenset({"nim-model"})),
        }
