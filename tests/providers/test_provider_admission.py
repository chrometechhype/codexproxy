...
from providers.failure_policy import ProviderRecoveryExhausted
...
async def test_recovery_traces_keep_the_logical_request_id() -> None:
    ...
    with patch("providers.admission.trace_event") as trace:
...
async def test_direct_exhaustion_trace_keeps_the_logical_request_id() -> None:
    ...
    with (
        patch("providers.admission.trace_event") as trace,
...
async def test_one_leader_backs_off_while_followers_coalesce() -> None:
    ...
    with patch(
        "providers.admission.asyncio.sleep",
