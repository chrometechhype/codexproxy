...
echo "uv:$*" >> "$CALL_LOG"
if [ "${{1:-}}" = "--version" ]; then
    if [ "${{CODEX_PROXY_RUNNING_PHASE:-}}" = "late" ]; then
        : > "$CODEX_PROXY_PROCESS_MARKER"
    fi
...
if [ "${{1:-}}" = "tool" ] && [ "${{2:-}}" = "install" ]; then
    if [ "$FAIL_STEP" = "cdx-install" ]; then
        exit 33
    fi
    mkdir -p "$FAKE_TOOL_BIN"
    cp "$FAKE_FIXTURES/cdx-command.sh" "$FAKE_TOOL_BIN/cdx-server"
...
""",
        ...
        self.env["CODEX_PROXY_PS_OUTPUT"] = process_line
    ...
    def run_interactive(
        ...
        *args: str,
        ...
        command = [
            "/bin/sh",
            "-c",
            'cat "$CODEX_PROXY_INSTALLER" | /bin/sh -s -- "$@"',
            "cdx-installer",
            *args,
        ]
        return subprocess.run(
...
if [ "${CODEX_PROXY_RUNNING_PHASE:-early}" = "late" ] && [ ! -e "$CODEX_PROXY_PROCESS_MARKER" ]; then
    exit 1
fi
case "$*" in
...
name=${0##*/}
echo "$name:$*" >> "$CALL_LOG"
if [ "$FAIL_STEP" = "cdx-verify" ]; then
    exit 36
fi
if [ "$name" = "cdx-desktop" ] && [ "${1:-}" = "--export-icon" ]; then
    [ "$FAIL_STEP" = "desktop-icon-export" ] && exit 37
    mkdir -p "$(dirname "$2")"
    printf 'fake icon\n' > "$2"
fi
if [ "$name" = "cdx-server" ] && [ "${1:-}" = "--version" ]; then
    echo "codexproxy 3.5.18"
fi
""",
    )
    _write_executable(
...
def test_install_sh_fresh_install_is_verified(posix_harness: PosixHarness) -> None:
    ...
    assert "CodexProxy is installed and verified." in result.stdout
    ...
    assert any(
        call.startswith(
            "uv:tool install --force --refresh-package codexproxy "
            "--python 3.14.0 codexproxy @ "
            "https://github.com/Alishahryar1/codexproxy/archive/refs/heads/main.zip"
        )
        for call in calls
    )
...
def test_install_sh_creates_native_macos_app_and_desktop_link(
    ...
    assert result.returncode == 0, result.stderr
    app = posix_harness.root / "home" / "Applications" / "CodexProxy.app"
    plist = app / "Contents" / "Info.plist"
    owner_file = app / "Contents" / ".codexproxy-owner"
    launcher = app / "Contents" / "MacOS" / "cdx-desktop"
    icon = app / "Contents" / "Resources" / "AppIcon.icns"
    desktop_link = posix_harness.root / "home" / "Desktop" / "CodexProxy.app"
    assert owner_file.read_text(encoding="utf-8").strip() == (
        "io.github.alishahryar1.codexproxy"
    )
    ...
    expected_command = str(tool_bin / "cdx-desktop").replace("'", "'\\''")
    ...
    assert any(
        call == f"cdx-desktop:--export-icon {icon}" for call in posix_harness.calls()
    )
...
def test_install_sh_stops_if_macos_icon_export_fails(
    ...
    assert not (posix_harness.root / "home" / "Desktop" / "CodexProxy.app").exists()
...
def test_install_sh_rejects_unowned_macos_app_bundle(
    ...
    assert result.returncode != 0
    assert "not managed by CodexProxy" in result.stderr
    assert plist.read_text(encoding="utf-8") == "foreign app"
    assert not (contents / ".codexproxy-owner").exists()
...
def test_install_sh_preserves_unrelated_macos_desktop_link(
    ...
    unrelated.mkdir()
    desktop_link = desktop / "CodexProxy.app"
...
def test_install_sh_continues_when_pi_is_not_installed(
    ...
    assert "cdx-server:--version" in calls
...
def test_install_sh_stops_without_success_on_each_failure(
    ...
    forbidden = {
        ...
        "cdx-install": "uv:tool update-shell",
        "path-update": "uv:tool dir --bin",
        "cdx-missing": "cdx-server:--version",
    }.get(failure)
...
def test_install_sh_voice_flags_only_change_fcc_spec(
    ...
    assert result.returncode == 0, result.stderr
    assert any(
        "--torch-backend cu130 codexproxy[voice,voice_local] @ "
        "https://github.com/Alishahryar1/codexproxy/archive/refs/heads/main.zip" in call
...
def test_install_sh_rejects_running_fcc_before_mutation(
    ...
    posix_harness.env["CODEX_PROXY_RUNNING_COMMAND"] = command_name
...
def test_install_sh_rechecks_for_fcc_process_before_tool_replacement(
    ...
    posix_harness.add_client("pi")
    posix_harness.add_uv("0.11.16")
    posix_harness.env["CODEX_PROXY_RUNNING_COMMAND"] = "cdx-server"
    posix_harness.env["CODEX_PROXY_RUNNING_PHASE"] = "late"

    result = posix_harness.run()

    assert result.returncode != 0
    assert "cdx-server (PID 4242)" in result.stderr
...
def test_install_sh_ignores_similarly_named_process(
    ...
    posix_harness.env["CODEX_PROXY_RUNNING_COMMAND"] = "cdx-server-helper"
...
@pytest.mark.parametrize(
    ...
    (
        ("codexproxy", "/home/user/.local/bin/codexproxy"),
        ("cdx-server", "/usr/bin/python3 /home/user/.local/bin/cdx-server"),
    ),
)
    ...
    script = "\n".join(
        (
            ...
            (
                ...
                "-DesktopCommand $env:CODEX_PROXY_TEST_DESKTOP_COMMAND "
                "-IconPath $env:CODEX_PROXY_TEST_ICON_PATH"
            ),
        )
    )
...
def _create_windows_shortcut(
    ...
    env = os.environ | {
        "CODEX_PROXY_TEST_SHORTCUT": str(shortcut_path),
        "CODEX_PROXY_TEST_TARGET": str(target_path),
    }
    subprocess.run(
        [
            powershell,
            ...
            (
                ...
                "$shortcut = $shell.CreateShortcut($env:CODEX_PROXY_TEST_SHORTCUT); "
                "$shortcut.TargetPath = $env:CODEX_PROXY_TEST_TARGET; "
                "$shortcut.Save()"
            ),
        ],
        check=True,
...
exit /b 59
:version
if "%CODEX_PROXY_RUNNING_PHASE%"=="late" type nul > "%CODEX_PROXY_PROCESS_MARKER%"
...
def powershell_harness(
    ...
    (fixtures / "cdx-command.cmd").write_text(
        """@echo off
for %%I in ("%~f0") do set "CODEX_PROXY_NAME=%%~nI"
echo %CODEX_PROXY_NAME%:%*>>"%CALL_LOG%"
if "%FAIL_STEP%"=="cdx-verify" exit /b 55
if "%CODEX_PROXY_NAME%"=="cdx-desktop" if "%1"=="--export-icon" if "%FAIL_STEP%"=="desktop-icon-export" exit /b 56
if "%CODEX_PROXY_NAME%"=="cdx-desktop" if "%1"=="--export-icon" (
    if not exist "%~dp2" mkdir "%~dp2"
    echo fake icon>"%~2"
)
if "%CODEX_PROXY_NAME%"=="cdx-server" if "%1"=="--version" echo codexproxy 3.5.18
exit /b 0
""",
        encoding="utf-8",
    )
...
function Get-Process {
    ...
    if ([string]::IsNullOrWhiteSpace($env:CODEX_PROXY_RUNNING_COMMAND)) {
        return
    }
    if (
        $env:CODEX_PROXY_RUNNING_PHASE -eq "late" -and
        -not (Test-Path -LiteralPath $env:CODEX_PROXY_PROCESS_MARKER)
    ) {
        return
    }
    foreach ($requestedName in $Name) {
        if ($requestedName -eq $env:CODEX_PROXY_RUNNING_COMMAND) {
...
def test_install_ps1_stops_if_windows_icon_export_fails(
    ...
    home = Path(powershell_harness.env["USERPROFILE"])
    assert not (home / "Desktop" / "CodexProxy.lnk").exists()
...
def test_install_ps1_preserves_unowned_desktop_shortcut(
    ...
    desktop_shortcut = (
        Path(powershell_harness.env["USERPROFILE"]) / "Desktop" / "CodexProxy.lnk"
    )
    ...
    assert result.returncode == 0, result.stderr
    assert "not managed by CodexProxy" in result.stdout
...
def test_install_ps1_rejects_running_fcc_before_mutation(
    ...
    powershell_harness.env["CODEX_PROXY_RUNNING_COMMAND"] = command_name
...
def test_install_ps1_rechecks_for_fcc_process_before_tool_replacement(
    ...
    powershell_harness.add_client("pi")
    powershell_harness.add_uv("0.11.16")
    powershell_harness.env["CODEX_PROXY_RUNNING_COMMAND"] = "cdx-server"
    powershell_harness.env["CODEX_PROXY_RUNNING_PHASE"] = "late"
...
def test_install_ps1_ignores_similarly_named_process(
    ...
    powershell_harness.env["CODEX_PROXY_RUNNING_COMMAND"] = "cdx-server-helper"
...
def test_installers_use_native_clients_and_single_python_selection() -> None:
    ...
    for text in (shell, powershell):
        for command_name in CODEX_PROXY_COMMANDS:
