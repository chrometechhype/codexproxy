...
""",
    ...
    app = home / "Applications" / "CodexProxy.app"
    contents = app / "Contents"
    contents.mkdir(parents=True)
    (contents / ".codexproxy-owner").write_text(
        "io.github.alishahryar1.codexproxy\n",
        encoding="utf-8",
    )
    desktop = home / "Desktop"
    desktop.mkdir()
    (desktop / "CodexProxy.app").symlink_to(app, target_is_directory=True)
...
def test_uninstall_sh_preserves_unrelated_macos_desktop_link(
    ...
    desktop_link = posix_uninstall_harness.home / "Desktop" / "CodexProxy.app"
    desktop_link.unlink()
...
def test_uninstall_sh_preserves_unowned_macos_app_bundle(
    ...
    app = posix_uninstall_harness.home / "Applications" / "CodexProxy.app"
...
if "%FAIL_STEP%"=="missing" echo Tool `codexproxy` is not installed 1>&2 & exit /b 2
if "%FAIL_STEP%"=="stale-entrypoint" echo Tool `codexproxy` is not installed 1>&2 & exit /b 2
...
$installer = [scriptblock]::Create([IO.File]::ReadAllText($env:CODEX_PROXY_UNINSTALLER))
    ...
    env.update(
        {
            ...
            "CALL_LOG": str(log),
            "FAKE_TOOL_BIN": str(tool_bin),
            "CODEX_PROXY_UNINSTALLER": str(_repo_root() / "scripts" / "uninstall.ps1"),
            "FAIL_STEP": "",
            "UNINSTALL_DRY_RUN": "0",
        }
    )
...
def test_uninstallers_guard_running_commands_and_preserve_shared_owners() -> None:
    ...
    for text in (shell, powershell):
        for command in CODEX_PROXY_COMMANDS:
